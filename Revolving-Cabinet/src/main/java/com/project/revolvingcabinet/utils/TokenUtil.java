...
public class TokenUtil {
    /** logger */
    private static Logger logger = LoggerFactory.getLogger(TokenUtil.class.getName());

    /** 设置token的有效时间为30天*/
    private static final long TOKEN_EMPIRE_TIME = 7 * 24 * 60 * 60 * 1000;
    /** 设置生成所需要的密钥盐 */
    private static final String TOKEN_SECRET = "Revolving-Cabinet";
    ...
    public static String getToken(String userId) {
        ...
        try {
            ...
            logger.error(Messages.getErrorMsg(Messages.MSG_E_LOG_005));
            jwtException.printStackTrace();
            throw new JWTCreationException(Messages.getErrorMsg(Messages.MSG_E_LOG_005), jwtException);
        } catch (IllegalArgumentException illException) {
            logger.error(Messages.getErrorMsg(Messages.MSG_E_LOG_005));
            illException.printStackTrace();
            throw new IllegalArgumentException(Messages.getErrorMsg(Messages.MSG_E_LOG_005));
        }
        return token;
    }
